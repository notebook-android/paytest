...
//    向线上服务器同步数据
    ...
    private void init_get_from_server(){
        new Thread(new Runnable() {
            @Override
            public void run() {
                OkHttpClient client = new OkHttpClient();
                            ...
                            public void run() {
                                ...
                                try{
                                    ...
                                    for(int i = 0;i < jsonArray.length();i++){
                                        ...
                                        pay = new Pay();
                                        pay.setId(Long.parseLong(jsonObject.getString("id")));
                                        pay.setName(jsonObject.getString("item_spend"));
                                        pay.setMoney(jsonObject.getDouble("money"));
                                        pay.setDay(jsonObject.getInt("day"));
                                        pay.setMonth(jsonObject.getInt("month"));
                                        pay.setYear(jsonObject.getInt("year"));
                                        pay.setUploaded(true);
                                        pay.save();
                                        pays.add(pay);
                                    }
                                ...
                                recyclerView.scrollToPosition(0);
                                refreshSum();
                                Toast.makeText(MainActivity.this, "成功同步", Toast.LENGTH_SHORT).show();
                            }
                        });
                    }
                } catch (IOException e) {
